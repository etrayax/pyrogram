...
class MessageReactionUpdated(Object):
    ...
    def _parse(
        ...
        if utils.get_peer_id(update.peer) > 0:
            chat = types.Chat._parse_user_chat(client, users[raw_peer_id])

        else:
            chat = types.Chat._parse_chat_chat(client, chats[raw_peer_id])

        raw_peer_id = utils.get_raw_peer_id(update.actor)
        if utils.get_peer_id(update.actor) > 0:
            user = types.User._parse(client, users[raw_peer_id])

        else:
            actor_chat = types.Chat._parse_channel_chat(client, chats[raw_peer_id])
        ...
        return MessageReactionUpdated(
            ...
            old_reaction=[
                types.ReactionType._parse(
                    client,
                    re
                ) for re in update.old_reactions
            ],
            new_reaction=[
                types.ReactionType._parse(
                    client,
                    re
                ) for re in update.new_reactions
            ],
        )
